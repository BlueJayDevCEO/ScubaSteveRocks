...
import { ai, encode, LIVE_CHAT_SYSTEM_PROMPT } from '../services/geminiService';
...
export const VoiceChatView: React.FC<VoiceChatViewProps> = ({
  ...
  // Simple helper to make Steve talk using browser TTS
  ...
  const handleStart = useCallback(async () => {
    ...
    try {
      ...
      const sessionPromise = ai.live.connect({
        // Use a stable live model – text modality
        ...
        callbacks: {
          ...
          onmessage: async (msg: LiveServerMessage) => {
            ...
            if (inputT) {
              setTranscript((prev) => [...prev, { role: 'user', text: inputT }]);
            }

            if (outputT) {
              setTranscript((prev) => [...prev, { role: 'model', text: outputT }]);
              speakText(outputT); // 🔊 Steve talks here
            }
          },
        ...
        config: {
          ...
          responseModalities: [Modality.TEXT], // text replies
      ...
      return (
        <div className="flex-1 p-4 overflow-y-auto">
          {currentBriefingResult.output?.transcript?.length ? (
            currentBriefingResult.output.transcript.map((m, i) => <TranscriptBubble key={i} message={m} />)
          ) : (
