...
const TESTS: DiagnosticResult[] = [
    ...
    {
        id: "storage_cxn",
        label: "Cloud Storage (Images)",
        status: "pending",
        message: "Waiting...",
    },
    {
        id: "ai_neural_link",
        label: "Scuba Steve Brain (LLM)",
        status: "pending",
        message: "Waiting...",
    },
    {
        id: "vision_core",
        label: "Marine ID Vision System",
        status: "pending",
        message: "Waiting...",
    },
    {
        id: "color_engine",
        label: "Color Correction Engine",
        status: "pending",
        message: "Waiting...",
    },
    {
        id: "trip_planner",
        label: "Trip Planner Logistics",
        status: "pending",
        message: "Waiting...",
    },
    {
        id: "voice_engine",
        label: "Voice Chat / Audio I/O",
        status: "pending",
        message: "Waiting...",
    },
    {
        id: "physics_core",
        label: "Dive Physics Kernel",
        status: "pending",
        message: "Waiting...",
    },
    {
        id: "mod_calc",
        label: "MOD / O2 Toxicity Logic",
        status: "pending",
        message: "Waiting...",
    },
    {
        id: "sac_rate",
        label: "Gas Consumption Logic",
        status: "pending",
        message: "Waiting...",
    },
    {
        id: "ead_calc",
        label: "Nitrox EAD Algorithms",
        status: "pending",
        message: "Waiting...",
    },
    {
        id: "boyles_law",
        label: "Pressure/Volume Physics",
        status: "pending",
        message: "Waiting...",
    },
];

export const SystemDiagnostics: React.FC<{ onBack: () => void }> = ({
    onBack,
}) => {
    ...
    useEffect(() => {
        ...
        const runTest = async (index: number) => {
            ...
            setResults((prev) =>
                prev.map((r, i) =>
                    i === index
                        ? { ...r, status: "running", message: "Calibrating..." }
                        : r,
                ),
            );
            ...
            try {
                ...
                switch (current.id) {
                    ...
                    case "mod_calc": {
                        ...
                        passed =
                            mod !== null &&
                            Math.abs((mod ?? 0) - 33.75) < 0.1;
                        msg = passed
                            ? `Calibrated (EAN32 = ${mod?.toFixed(1)}m)`
                            : "Calc Error";
                        break;
                    }
                    case "sac_rate": {
                        ...
                        passed =
                            sac !== null &&
                            Math.abs((sac?.sac ?? 0) - 30) < 0.1;
                        msg = passed
                            ? `Flow Verified (${sac?.sac.toFixed(0)} L/min)`
                            : "Calc Error";
                        break;
                    }
                    case "ead_calc": {
                        const ead = calculateEAD(30, 32);
                        passed = ead !== null && (ead ?? 0) < 30;
                        msg = passed
                            ? "N2 Loading Correct"
                            : "Calc Error";
                        break;
    ...
    return (
        <CalculatorShell title="System Calibration" onBack={onBack}>
            <div className="bg-black rounded-xl p-6 font-mono text-sm border border-light-accent/30 dark:border-dark-accent/30 shadow-2xl overflow-hidden relative">
                {/* Scan Line Animation */}
                {!isComplete && (
                    <div className="absolute top-0 left-0 w-full h-1 bg-green-500/50 shadow-[0_0_15px_rgba(34,197,94,0.8)] animate-[scan_2s_linear_infinite] z-10 pointer-events-none" />
                )}
                        ...
                        <div>
                            <p className="text-gray-400 text-xs">
                                ...
                                <span className="text-white">
                                    SCUBA_STEVE_CORE_V2.1
                                </span>
                            </p>
                            <p className="text-gray-400 text-xs">
                                MODE:{" "}
                                <span className="text-light-accent dark:text-dark-accent">
                                    FULL_DIAGNOSTIC_SWEEP
                                </span>
                            </p>
                        </div>
                ...
                <div className="space-y-3">
                    {results.map((test) => (
                        <div
                            key={test.id}
                            className="flex items-center justify-between border-b border-gray-900/50 pb-1 last:border-0"
                        >
                ...
                {isComplete && (
                    <div className="mt-8 text-center animate-fade-in">
                        <p className="text-light-text/80 dark:text-dark-text/80 mb-6 text-lg font-medium">
                            Calibration successful. Your AI Dive Buddy is fully
                            operational.
                        </p>
                        <button
                            onClick={onBack}
                            className="bg-gradient-to-r from-light-accent to-light-primary-end dark:from-dark-accent dark:to-dark-primary-end text-white px-8 py-3 rounded-full font-bold hover:scale-105 transition-transform shadow-lg"
                        >
                            Return to Tools
                        </button>
                    </div>
